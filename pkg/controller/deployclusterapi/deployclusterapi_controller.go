...
const (
	...
	controllerLogName = "deployclusterapi"

	nodeConfigPlaybook = "playbooks/cluster-api-prep/deploy-cluster-api.yaml"
	// jobType is the type of job run by this controller.
	jobType = "deployclusterapi"
)
...
func NewController(
	...
	if kubeClient != nil && kubeClient.CoreV1().RESTClient().GetRateLimiter() != nil {
		metrics.RegisterMetricAndTrackRateLimiterUsage("clusteroperator_kubeclusterapi controller", kubeClient.CoreV1().RESTClient().GetRateLimiter())
	}
...
func (c *Controller) Run(workers int, stopCh <-chan struct{}) {
	...
	c.logger.Infof("Starting deployclusterapi controller")
	defer c.logger.Infof("Shutting down deployclusterapi controller")

	if !controller.WaitForCacheSync("deployclusterapi", stopCh, c.machineSetsSynced, c.jobsSynced) {
		c.logger.Errorf("Could not sync caches for deployclusterapi controller")
		return
	}
...
func (s *jobSyncStrategy) DoesOwnerNeedProcessing(owner metav1.Object) bool {
	...
	if machineSet.Status.ClusterAPIInstalledJobGeneration != machineSet.Generation {
		msLog.WithFields(log.Fields{
			"ClusterAPIInstalledJobGeneration": machineSet.Status.ClusterAPIInstalledJobGeneration,
			"Generation":                       machineSet.Generation,
		}).Debug("machine set generation has changed, processing required")
		return true
	}
	return false
}
...
func (s *jobSyncStrategy) GetLastJobSuccess(owner metav1.Object) *time.Time {
	...
	if machineSet.Status.ClusterAPIInstalledTime != nil {
		return &machineSet.Status.ClusterAPIInstalledTime.Time
	}
	return nil
}
...
func (s *jobSyncStrategy) OnJobCompletion(owner metav1.Object, job *v1batch.Job, succeeded bool) {
	...
	msLog.Debug("running")
	machineSet.Status.ClusterAPIInstalled = succeeded
	machineSet.Status.ClusterAPIInstalledJobGeneration = machineSet.Generation

	msLog.WithFields(log.Fields{
		"job":       job.Name,
		"completed": job.Status.CompletionTime}).Infoln("found successful job")
	machineSet.Status.ClusterAPIInstalledTime = job.Status.CompletionTime
...
func convertJobSyncConditionType(conditionType controller.JobSyncConditionType) clusteroperator.MachineSetConditionType {
	...
	case controller.JobSyncProcessing:
		return clusteroperator.MachineSetClusterAPIInstalling
	case controller.JobSyncProcessed:
		return clusteroperator.MachineSetClusterAPIInstalled
	case controller.JobSyncProcessingFailed:
		return clusteroperator.MachineSetClusterAPIInstallationFailed
	default:
