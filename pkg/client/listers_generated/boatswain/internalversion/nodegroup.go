...
type NodeGroupLister interface {
	...
	List(selector labels.Selector) (ret []*boatswain.NodeGroup, err error)
...
// List lists all NodeGroups in the indexer.
func (s *nodeGroupLister) List(selector labels.Selector) (ret []*boatswain.NodeGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*boatswain.NodeGroup))
	})
	return ret, err
}
...
type NodeGroupNamespaceLister interface {
	...
	Get(name string) (*boatswain.NodeGroup, error)
...
// List lists all NodeGroups in the indexer for a given namespace.
func (s nodeGroupNamespaceLister) List(selector labels.Selector) (ret []*boatswain.NodeGroup, err error) {
...
// Get retrieves the NodeGroup from the indexer for a given namespace and name.
func (s nodeGroupNamespaceLister) Get(name string) (*boatswain.NodeGroup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(boatswain.Resource("nodegroup"), name)
	}
	return obj.(*boatswain.NodeGroup), nil
}
