...
type ClusterVersionLister interface {
	...
	List(selector labels.Selector) (ret []*clusteroperator.ClusterVersion, err error)
...
// List lists all ClusterVersions in the indexer.
func (s *clusterVersionLister) List(selector labels.Selector) (ret []*clusteroperator.ClusterVersion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*clusteroperator.ClusterVersion))
	})
	return ret, err
}
...
type ClusterVersionNamespaceLister interface {
	...
	Get(name string) (*clusteroperator.ClusterVersion, error)
...
// List lists all ClusterVersions in the indexer for a given namespace.
func (s clusterVersionNamespaceLister) List(selector labels.Selector) (ret []*clusteroperator.ClusterVersion, err error) {
...
// Get retrieves the ClusterVersion from the indexer for a given namespace and name.
func (s clusterVersionNamespaceLister) Get(name string) (*clusteroperator.ClusterVersion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(clusteroperator.Resource("clusterversion"), name)
	}
	return obj.(*clusteroperator.ClusterVersion), nil
}
