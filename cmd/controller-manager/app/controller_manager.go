...
// Package app implements a server that runs the boatswain controllers.
package app
...
import (
	...
	_ "github.com/staebler/boatswain/pkg/api"
...
func NewControllerManagerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `The boatswain controller manager is a daemon that embeds
the core control loops shipped with the boatswain.`,
...
const controllerManagerAgentName = "boatswain-controller-manager"
const controllerDiscoveryAgentName = "boatswain-controller-discovery"

var boatswainGVR = schema.GroupVersionResource{Group: "boatswain.k8s.io", Version: "v1alpha1", Resource: "clusterservicebrokers"}

// Run runs the boatswain controller-manager; should never exit.
func Run(controllerManagerOptions *options.ControllerManagerServer) error {
	...
	glog.V(4).Infof("Building boatswain kubeconfig for url: %v\n", controllerManagerOptions.BoatswainAPIServerURL)

	var boatswainKubeconfig *rest.Config
	// Build the boatswain kubeconfig / clientBuilder
	if controllerManagerOptions.BoatswainAPIServerURL == "" && controllerManagerOptions.BoatswainKubeconfigPath == "" {
		...
		glog.V(4).Infof("Using inClusterConfig to talk to boatswain API server -- make sure your API server is registered with the aggregator")
		boatswainKubeconfig, err = rest.InClusterConfig()
	} else {
		boatswainKubeconfig, err = clientcmd.BuildConfigFromFlags(
			controllerManagerOptions.BoatswainAPIServerURL,
			controllerManagerOptions.BoatswainKubeconfigPath)
	}
	if err != nil {
		// TODO: disambiguate API errors
		return fmt.Errorf("failed to get Boatswain client configuration: %v", err)
	}
	boatswainKubeconfig.Insecure = controllerManagerOptions.BoatswainInsecureSkipVerify
	...
	go func() {
		...
		apiAvailableChecker := checkAPIAvailableResources{
			controller.SimpleClientBuilder{
				ClientConfig: boatswainKubeconfig,
			},
		}
	...
	run := func(stop <-chan struct{}) {
		boatswainClientBuilder := controller.SimpleClientBuilder{
			ClientConfig: boatswainKubeconfig,
		}
		...
		err := StartControllers(controllerManagerOptions, k8sKubeconfig, boatswainClientBuilder, recorder, stop)
	...
	rl := resourcelock.EndpointsLock{
		EndpointsMeta: metav1.ObjectMeta{
			...
			Name:      "boatswain-controller-manager",
		},
		Client: leaderElectionClient.CoreV1(),
		LockConfig: resourcelock.ResourceLockConfig{
			Identity:      id + "-external-boatswain-controller",
...
// StartControllers starts all the controllers in the boatswain
// controller manager.
func StartControllers(s *options.ControllerManagerServer,
	coreKubeconfig *rest.Config,
	boatswainClientBuilder controller.ClientBuilder,
	recorder record.EventRecorder,
	stop <-chan struct{}) error {

	// Get available boatswain resources
	glog.V(5).Info("Getting available resources")
	availableResources, err := getAvailableResources(boatswainClientBuilder)
	if err != nil {
		return err
	}
	...
	// Launch boatswain controller
	if availableResources[boatswainGVR] {
		glog.V(5).Infof("Creating shared informers; resync interval: %v", s.ResyncInterval)

		// Build the informer factory for boatswain resources
		informerFactory := boatswaininformers.NewSharedInformerFactory(
			boatswainClientBuilder.ClientOrDie("shared-informers"),
			s.ResyncInterval,
		)
		// All shared informers are v1alpha1 API level
		boatswainSharedInformers := informerFactory.Boatswain().V1alpha1()

		glog.V(5).Infof("Creating controller")
		boatswainController, err := controller.NewController(
			coreClient,
			boatswainClientBuilder.ClientOrDie(controllerManagerAgentName).BoatswainV1alpha1(),
		...
		go boatswainController.Run(s.ConcurrentSyncs, stop)

		glog.V(1).Info("Starting shared informers")
		informerFactory.Start(stop)
	} else {
		return fmt.Errorf("unable to start boatswain controller: API GroupVersion %q is not available; found %#v", boatswainGVR, availableResources)
	}
...
// Boatswain APIServer is contactable.
...
func (c checkAPIAvailableResources) Check(_ *http.Request) error {
	glog.Info("Health-checking connection with boatswain API server")
	availableResources, err := getAvailableResources(c.boatswainClientBuilder)
	if err != nil {
		return err
	}
	if !availableResources[boatswainGVR] {
		return fmt.Errorf("failed to get API GroupVersion %q; found: %#v", boatswainGVR, availableResources)
	}
	return nil
}
