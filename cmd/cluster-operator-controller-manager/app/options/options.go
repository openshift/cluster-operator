...
func NewCMServer() *CMServer {
	s := CMServer{
		ControllerManagerConfiguration: componentconfig.ControllerManagerConfiguration{
			...
			ConcurrentRoute53HostedZoneSyncs: defaultConcurrentSyncs,
...
func (s *CMServer) AddFlags(fs *pflag.FlagSet, allControllers []string, disabledByDefaultControllers []string) {
	...
	fs.Int32Var(&s.ConcurrentRoute53HostedZoneSyncs, "concurrent-route53hostedzone-syncs", s.ConcurrentRoute53HostedZoneSyncs, "The number of route53hostedzones we can sync to AWS concurrently. Larger number = more responsive sync jobs, but more AWS API calls, CPU (and network ) load")
