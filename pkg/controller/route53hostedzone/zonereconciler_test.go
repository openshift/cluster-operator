	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			expectedZoneReconciler := &ZoneReconciler{
				...
				clusteroperatorClient: mocks.fakeClusteropClient,
				logger:                log.WithField("controller", controllerName),
				clusterOperatorAwsClient: mocks.mockAWSClient,
			}
			...
			zr, err := NewZoneReconciler(
				...
				expectedZoneReconciler.clusteroperatorClient,
				expectedZoneReconciler.logger,
				expectedZoneReconciler.clusterOperatorAwsClient,
			)
