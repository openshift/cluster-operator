...
func buildGenericConfig(s *BoatwsainServerOptions) (*genericapiserver.RecommendedConfig, *boatswainConfig, error) {
	// check if we are running in standalone mode (for test scenarios)
	inCluster := !s.StandaloneMode
	if !inCluster {
		glog.Infof("boatswain is in standalone mode")
	}
...
func buildAdmission(s *BoatwsainServerOptions,
