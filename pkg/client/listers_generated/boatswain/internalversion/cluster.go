...
type ClusterLister interface {
	...
	List(selector labels.Selector) (ret []*boatswain.Cluster, err error)
...
// List lists all Clusters in the indexer.
func (s *clusterLister) List(selector labels.Selector) (ret []*boatswain.Cluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*boatswain.Cluster))
	})
	return ret, err
}
...
type ClusterNamespaceLister interface {
	...
	Get(name string) (*boatswain.Cluster, error)
...
// List lists all Clusters in the indexer for a given namespace.
func (s clusterNamespaceLister) List(selector labels.Selector) (ret []*boatswain.Cluster, err error) {
...
// Get retrieves the Cluster from the indexer for a given namespace and name.
func (s clusterNamespaceLister) Get(name string) (*boatswain.Cluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(boatswain.Resource("cluster"), name)
	}
	return obj.(*boatswain.Cluster), nil
}
