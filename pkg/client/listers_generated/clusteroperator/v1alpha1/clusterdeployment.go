...
type ClusterDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ClusterDeployment, err error)
...
// List lists all ClusterDeployments in the indexer.
func (s *clusterDeploymentLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ClusterDeployment))
	})
	return ret, err
}
...
type ClusterDeploymentNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.ClusterDeployment, error)
...
// List lists all ClusterDeployments in the indexer for a given namespace.
func (s clusterDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterDeployment, err error) {
...
// Get retrieves the ClusterDeployment from the indexer for a given namespace and name.
func (s clusterDeploymentNamespaceLister) Get(name string) (*v1alpha1.ClusterDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clusterdeployment"), name)
	}
	return obj.(*v1alpha1.ClusterDeployment), nil
}
