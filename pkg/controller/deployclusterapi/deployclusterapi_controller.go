...
func (s *installStrategy) ReadyToInstall(cluster *clustop.CombinedCluster, masterMachineSet metav1.Object) bool {
	...
	return cluster.ClusterOperatorStatus.ClusterAPIInstalledJobClusterGeneration != cluster.Generation ||
		cluster.ClusterOperatorStatus.ClusterAPIInstalledJobMachineSetGeneration != masterMachineSet.GetGeneration()
}
...
func (s *installStrategy) OnInstall(succeeded bool, cluster *clustop.CombinedCluster, masterMachineSet metav1.Object, job *batchv1.Job) {
	cluster.ClusterOperatorStatus.ClusterAPIInstalled = succeeded
	cluster.ClusterOperatorStatus.ClusterAPIInstalledJobClusterGeneration = cluster.Generation
	cluster.ClusterOperatorStatus.ClusterAPIInstalledJobMachineSetGeneration = masterMachineSet.GetGeneration()
	cluster.ClusterOperatorStatus.ClusterAPIInstalledTime = job.Status.CompletionTime
}
...
func (s *installStrategy) GetLastJobSuccess(cluster *clustop.CombinedCluster) *time.Time {
	if cluster.ClusterOperatorStatus.ClusterAPIInstalledTime == nil {
		return nil
	}
	return &cluster.ClusterOperatorStatus.ClusterAPIInstalledTime.Time
}
