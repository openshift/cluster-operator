...
type NodeLister interface {
	...
	List(selector labels.Selector) (ret []*boatswain.Node, err error)
...
// List lists all Nodes in the indexer.
func (s *nodeLister) List(selector labels.Selector) (ret []*boatswain.Node, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*boatswain.Node))
	})
	return ret, err
}
...
type NodeNamespaceLister interface {
	...
	Get(name string) (*boatswain.Node, error)
...
// List lists all Nodes in the indexer for a given namespace.
func (s nodeNamespaceLister) List(selector labels.Selector) (ret []*boatswain.Node, err error) {
...
// Get retrieves the Node from the indexer for a given namespace and name.
func (s nodeNamespaceLister) Get(name string) (*boatswain.Node, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(boatswain.Resource("node"), name)
	}
	return obj.(*boatswain.Node), nil
}
